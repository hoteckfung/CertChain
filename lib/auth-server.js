  ...
  try {
    ...
    if (!user) {
      ...
      const newUserData = {
        ...
        role: ROLES.HOLDER,
        username: userData.username || `User_${walletAddress.substring(2, 8)}`,
...
// Role update function with cache invalidation
export async function updateUserRole(userId, newRole, adminUserId) {
  try {
    ...
    // Get updated user to clear cache
    const { data: updatedUser } = await mysql.getAllUsers();
    const user = updatedUser.find((u) => u.id === parseInt(userId));

    if (user) {
      // Clear cache for this user to force fresh data on next request
      clearUserCache(user.wallet_address);
...
export function canAccessRoute(userRole, routePath) {
  ...
  const routePermissions = {
    "/admin": [ROLES.ADMIN],
    "/issuer": [ROLES.ISSUER, ROLES.ADMIN],
    "/holder": [ROLES.HOLDER, ROLES.ISSUER, ROLES.ADMIN],
  };
